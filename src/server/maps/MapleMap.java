...
public final class MapleMap {
    ...
    private final LinkedList<MonsterStatusEffect> poisons1 = new LinkedList<MonsterStatusEffect>();
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        ...
        int zhuandan = (int) Math.floor(Math.random()*9+1);
        int chance = (int) Math.floor(Math.random()*1+1);
        int dianquan = (int) (mob.getStats().getLevel() * 0.2);
        if(this.channel == 2) {
            dianquan = dianquan * 2;
            chance = (int) Math.floor(Math.random()*1+1);
            if(zhuandan >= 9)
            if(mob.getStats().getLevel() - chr.getLevel() >= -60){
            Item toDrop;
            toDrop = new client.inventory.Item(5220040, (byte) 0, (short) 1, (byte) 0);
            chr.getMap().spawnItemDrop(chr, chr, toDrop, mob.getTruePosition(), true, true);
        }
        }
        if(this.channel == 3) {
            chance = (int) Math.floor(Math.random()*3+1);
            dianquan = dianquan * 3;
            if(zhuandan >= 8)
            if(mob.getStats().getLevel() - chr.getLevel() >= -60){
            Item toDrop;
            toDrop = new client.inventory.Item(5220040, (byte) 0, (short) chance, (byte) 0);
            chr.getMap().spawnItemDrop(chr, chr, toDrop, mob.getTruePosition(), true, true);
        }
        }
        chr.modifyCSPoints(2, dianquan);
        for (final MonsterDropEntry de : dropEntry) {
            ...
            if (Randomizer.nextInt(999999) < (int) (de.chance * chServerrate * chr.getDropMod() * (chr.getStat().dropBuff / 100.0) * (showdown / 100.0))) {
                ...
                if (droptype == 3) {
                    ...
                    int mesos = (int) (shu >= 8 ? (mob.getStats().getExp() * 0.3) :0);
                    if(this.channel == 2)
                        mesos *= 5;
                    if(this.channel == 3)
                        mesos *= 10;
                    if(chr.haveItem(5360042))
                        mesos *= 1.5;
                    if (mesos > 0) {
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
        ...
        if (mobid == 8810018 && mapid == 240060200) { // Horntail
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "经过无数次的挑战，终于击破了暗黑龙王的远征队！你们才是龙之林的真正英雄~"));
            charactersLock.readLock().lock();
            try {
            ...
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "经过无数次的挑战，终于击破了进阶暗黑龙王的远征队！你们才是龙之林的真正英雄~"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
...
//            FileoutputUtil.log(FileoutputUtil.Horntail_Log, MapDebug_Log());
            ...
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "经过无数次的挑战，终于击破了时间的宠儿－品克缤的远征队！你们才是时间神殿的真正英雄~"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
            ...
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "被黑魔法师黑化的希纳斯女皇终于被打倒! 混沌世界得以净化!"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
        ...
        if (type != null) {
            if (speedRunStart > 0 && speedRunLeader.length() > 0) {
                ...
                broadcastMessage(CWvsContext.serverNotice(5, speedRunLeader + "'带领的远征队，耗时: " + time + " 击败了 " + type.name() + "!"));
...
/*      */ 
/*      */   public final void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
    double jingying = Math.floor(Math.random()*99+1);
/* 1407 */     monster.setMap(this);
if (channel == 1) {//?????l??
                    monster.changeLevelmod(monster.getStats().getLevel(), 1);
        }
                if (channel == 2) {//?????l??
                    if(monster.getStats().isBoss() && !(monster.getMap().getId() >= 952000000 && monster.getMap().getId() <= 954050500)) {
                monster.changeLevelmod(monster.getStats().getLevel(), 1);
            } else
                    monster.changeLevelmod(monster.getStats().getLevel(), 10);
        }
        if (channel == 3) {//?????l??
            if(monster.getStats().isBoss() && !(monster.getMap().getId() >= 952000000 && monster.getMap().getId() <= 954050500)) {
                monster.changeLevelmod(monster.getStats().getLevel(), 1);
            } else
                    monster.changeLevelmod(monster.getStats().getLevel(), 100);
        }
