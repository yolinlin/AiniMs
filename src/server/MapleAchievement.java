...
public class MapleAchievement {
    ...
    public void finishAchievement(MapleCharacter chr) {
        ...
        if (notice && !chr.isGM()) {
            World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "[成就系统] 祝贺 " + chr.getName() + " 获得 '" + name + "' 和奖励 " + (GameConstants.GMS ? (reward/2) : reward) + " 点卷!"));
        } else {
            chr.getClient().getSession().write(CWvsContext.serverNotice(5, "[成就系统] 完成 '" + name+ "' 成就 获得"+(GameConstants.GMS ? (reward/2) : reward) + " 点卷."));
        }
    }
}