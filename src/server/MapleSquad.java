...
public class MapleSquad {
	...
	while (type.queue.get(ch).size() > 0 && ChannelServer.getInstance(ch).getMapleSquad(type) == null) {
	    ...
	    if (theirCh > 0) {
		    ...
		    type.queuedPlayers.get(ch).add(new Pair<String, String>(nextPlayerId, "不在地图"));
		}
	    } else {
		...
		type.queuedPlayers.get(ch).add(new Pair<String, String>(nextPlayerId, "没有在线"));
	    }
	}
    }
    ...
    public String getNextPlayer() {
        ...
        sb.append("#b").append(type.queue.get(ch).size()).append(" #k ").append("名单 : \n\r ");
        int i = 0;
