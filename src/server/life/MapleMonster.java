...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private int stolen = -1, jingying; //monster can only be stolen ONCE
    private boolean shouldDropItem = false, killed = false;

    public final void changeLevelmod(final int newLevel, int multipler) {//�����l��
    ...
    public final void damage(final MapleCharacter from, final long damage, final boolean updateAttackTime, final int lastSkill) {
        String str = ""+ this.getHp();
        if (from == null || damage <= 0 || !isAlive()) {
            return;
        }
        if(lastSkill == 5221016 && !this.getStats().isBoss() || lastSkill == 5721006 && !this.getStats().isBoss()){
            damage(from, getHp(), false);
        ...
        if(from.getshanghai() == 1)
            from.getClient().getSession().write(CField.sendHint(""+ str +" - "+ damage +" = "+ this.getHp() + "\n  还需"+ this.getHp() / damage +" 次攻击", 250, 5));
    }
    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            int dianquan = (int) (getLevel()*0.2);
            if (attacker.getClient().getChannel() == 2){
                dianquan = dianquan * 2;
            }
            if (attacker.getClient().getChannel() == 3){
                dianquan = dianquan * 3;
            }
            //if(getStats().getLevel() - attacker.getLevel() >= -20){
            attacker.modifyCSPoints(2, dianquan);
            //}
        ...
        if(attacker.getClient().getChannel() == 2) {
                aas = 3;
            } else if(attacker.getClient().getChannel() == 3) {
                aas = 10;
            }
        for(int i = 0;i < aas;i++) {
