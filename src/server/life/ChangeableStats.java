...
public class ChangeableStats extends OverrideMonsterStats {
    ...
    public ChangeableStats(MapleMonsterStats stats, int newLevel, int beishu) {
         final double base = beishu;
        if ((int) Math.round(stats.getHp() * base) > Integer.MAX_VALUE) {
            hp = (int) Math.round(stats.getHp());
        } else {
            hp = (int) Math.round(stats.getHp() * base);
        }
        mp = (int) Math.round(stats.getMp() * base);
        if (mp > Integer.MAX_VALUE) {
            mp = Integer.MAX_VALUE;
        }
        if ((int) Math.round(stats.getExp() * base) > Integer.MAX_VALUE) {
            exp = (int) Math.round(stats.getExp());
        } else {
            exp = (int) Math.round(stats.getExp() * base);
        }
        ...
        pushed = (int) Math.round(stats.getPushed());
