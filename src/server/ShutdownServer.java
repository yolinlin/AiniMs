...
public class ShutdownServer implements ShutdownServerMBean {
	...
	if (mode == 0) {
	    int ret = 0;
	    World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "JudoMS World is going to Shutdown soon. Please kindly log off now for the meantime."));
            for (ChannelServer cs : ChannelServer.getAllInstances()) {
                cs.setShutdown();
				cs.setServerMessage("JudoMS World is going to Shutdown soon. Please kindly log off now for the meantime.");
	    ...
	    mode++;
            ...
            try {
	        World.Broadcast.broadcastMessage(CWvsContext.serverNotice(0, "JudoMS World is going to Shutdown now. Please kindly log off now for the meantime."));
