...
public class PlayerHandler {
    ...
    public static final void SpecialMove(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
         ...
         * 589
         */ 
         if (skillid == 20021110 || skillid == 80001040) { // 幻影回归      
            c.getPlayer().saveLocation(SavedLocationType.JINGLING);
            c.getPlayer().changeMap(101050000, 0);
                     return;
        }
         if (skillid == 20031203) { // 幻影回归      
             if(c.getPlayer().getMapId() >= 150000000 && c.getPlayer().getMapId() <= 150010412){
                 c.getPlayer().dropMessage(5, "你都在家里你飞个卵啊.");
             }
            c.getPlayer().saveLocation(SavedLocationType.FEICHUAN);
            c.getPlayer().changeMap(150000000 , 0);
                     return;
        }
         if (skillid == 23111008) {
            /*
            ...
            case 33101005:
                 ...
                 */ if (mob != null) {
                     ...
                     */ if (success) {
                        ...
                        /*
                         * 714
                         */ mob.damage(chr, mob.getHp(), false);
                    } else {
                        /*
                         * 716
                         */ chr.dropMessage(5, "怪物有太多HP无法吞噬.");
                    }
                } else {
                    /*
                     * 719
                     */ chr.dropMessage(5, "没有怪物.");
                }
                /*
                 * 721
                 */ c.getSession().write(CWvsContext.enableActions());
    ...
    public static final void closeRangeAttack(LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        if (attack.skill != 0) {
            ...
            if ((effect.getCooldown(chr) > 0) && (!chr.isGM()) && (!energy) && (attack.skill != 24121005)) {
         ...
         */ if (slea.available() != 0L) {
             ...
             */ if ((targetid != -1) && (!chr.isAlive())) {
                 ...
                 */ if (chr.getPyramidSubway() != null) {
                    /*
                     * 1392
                     */ chr.getStat().setHp(chr.getMaxHp(), chr);
                    /*
                     * 1393
