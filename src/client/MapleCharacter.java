...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
    ...
    private byte gmLevel, gender, initialSpawnPoint, skinColor, guildrank = 5, allianceRank = 5,
            world, fairyExp, numClones, subcategory, cardStack, runningStack, pendantExp1 = 0;
    ...
    private int accountid, id, meso, exp, hair, face, demonMarking, elf, mapid, fame, pvpExp, pvpPoints, totalWins, totalLosses,
            ...
            currentrep, totalrep, coconutteam, followid, battleshipHP, gachexp, challenge, guildContribution = 0, remainingAp, honourExp, honourLevel, shanghai, zidong;
    private Point old;
    ...
    private ScheduledFuture<?> pendantOfSpirit1 = null;
    /*
...
// The sp isnt fixed oh hold on
 ...
 public void totemfix(){
        ...
        short dst = getInventory(MapleInventoryType.EQUIP).getNextFreeSlot();
              MapleInventoryManipulator.unequip(this.getClient(), (byte)-5000, dst);
             MapleInventoryManipulator.equip(this.getClient(), (byte)-5000, dst);
             return;
         } else if (totem1 != null){
                short dst = getInventory(MapleInventoryType.EQUIP).getNextFreeSlot();
              MapleInventoryManipulator.unequip(this.getClient(), (byte)-5001, dst);
             MapleInventoryManipulator.equip(this.getClient(), (byte)-5001, dst);
             return;
             } else if (totem2 != null){
                         short dst = getInventory(MapleInventoryType.EQUIP).getNextFreeSlot();
              MapleInventoryManipulator.unequip(this.getClient(), (byte)-5002, dst);
             MapleInventoryManipulator.equip(this.getClient(), (byte)-5002, dst);
             return;
    }
 }    
    ...
    public void playerDead() {
        ...
        if (statss != null) {
            ...
            getStat().setHp(this.getMaxHp(), this);
            setStance(0);
       ...
       if (getGuildContribution() >= 14400) {
        ...
        if (GameConstants.isJett(job)) {
            switch (getLevel()) {
                ...
                setRemainingSp((getLevel() - 10) * 3 + 1);
        ...
        if (GameConstants.isMihile(job)) { // Mihile
            switch (getLevel()) {
                case 10:
                ...
                updateSingleStat(MapleStat.AVAILABLESP, getRemainingSp());
                    break;
                case 30:
                    MapleInventoryManipulator.addById(client, 1098001, (short) 1, "Scripted item: ");
                    changeJob((short) 5110);
                    break;
                case 70:
                    changeJob((short) 5111);
                    MapleInventoryManipulator.addById(client, 1098002, (short) 1, "Scripted item: ");
                    break;
                case 120:
                    changeJob((short) 5112);
                    MapleInventoryManipulator.addById(client, 1098003, (short) 1, "Scripted item: ");
                    break;
            }

        }
        
        if (GameConstants.isKOC(job) && job > 1000) { // KOC (Not Nobless) //1500, 1510, 1511, 1512
        ...
        if (GameConstants.isEvan(job)) { // 2218, 2217, 2216, 2215, 2214, 2213, 2212, 2211, 2210, 2200, 2001
            if (level >= 160 && job != 2218) {
                ...
                setRemainingAp((getLevel() - 1) * 5 + 8);
                updateSingleStat(MapleStat.AVAILABLEAP, getRemainingAp());
                setRemainingSp((getLevel() - 10) * 3 + 10);
                updateSingleStat(MapleStat.AVAILABLESP, getRemainingSp());
            }
        ...
        } else if (GameConstants.isCannon(job)) {//火炮手
            switch (getLevel()) {
                case 30:
                    changeJob((short) 530);
                    break;
        ...
        } else if (GameConstants.isMercedes(job)) {//双弩精灵
            switch (getLevel()) {
                case 10:
        ...
        if (GameConstants.isResist(job)) { //BattleMage
            switch (getJob()) {
                case 3200:
                case 3210:
                case 3211:
    ...
    public FameStatus canGiveFame(MapleCharacter from) {
        if (lastfametime >= System.currentTimeMillis() - 60 * 60 * 1000) {
...
public void withdrawMesosFromBank(long mesos) {
    ...
    public void equipPendantOfSpirit1() {
        if (pendantOfSpirit1 == null) {
            pendantOfSpirit1 = MapTimer.getInstance().register(new Runnable() {
                @Override
                public void run() {
                    if (pendantExp1 < 999) {
                        pendantExp1++;
                        message("每在线20分钟，就可以增加1倍经验, 目前倍数为 " + pendantExp1 + "00% 经验.");
                    } else {
                        pendantOfSpirit1.cancel(false);
                    }
                }
            }, 1200000); //20 min
        }
