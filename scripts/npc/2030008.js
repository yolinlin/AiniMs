...
function action(mode, type, selection) {
	...
	if (cm.getPlayerStat("LVL") >= 50) {
		...
		cm.sendOk("你需要完成扎昆任务才能进入……恩，我觉得你可以完成。");
		cm.dispose();
		return;
		...
		if (cm.haveItem(4001109)) {
		    cm.sendSimple("好吧，你看上去很可靠。\n\r你要做哪个? #b\r\n#L0#第一阶段：前往 未知废矿#l\r\n#L1#第二阶段：前往 火山心藏#l\r\n#L2#第三阶段：收集 僵尸丢失的金齿#l\r\n#L3#Enter the center of Lava.#l\r\n#l");
		} else {
		    cm.sendSimple("好吧，你看上去很可靠。\n\r你要做哪个? #b\r\n#L0#第一阶段：前往 未知废矿#l\r\n#L1#第二阶段：前往 火山心藏#l\r\n#L2#第三阶段：收集 僵尸丢失的金齿#l\r\n#l");
		}
	    }
	...
	if (selection == 0) { //ZPQ
		...
		cm.sendNext("你现在没有一个队伍啊。");
		cm.safeDispose();
		return;
	    }
	    else if (!cm.isLeader()) { //not party leader
		cm.sendNext("叫你的队长来跟我讲话吧.");
		cm.safeDispose();
		return;
	    }
	    else {
		...
		if (cm.haveItem(4031062))
		    ...
		    cm.sendNext("看来你还没有到这个阶段，你必须完成上一个阶段哦.");
		cm.dispose();
	    }
